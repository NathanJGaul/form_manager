...
export class JCC2UserQuestionnaireV2 {
  static create(): ProgrammaticTemplate {
    ...
    builder.section('MOS 1.1.2: Tagging objects of interest and enabling correlation of objects')
    ...
    builder.section('MOPS 1.1.4/1.2.3/1.3.6/2.3.3: Reporting and Data Export');
    allJcc2Apps.forEach(app => {
    ...
    eventCreationApps.forEach(app => {
        builder.section(`MOP 1.3.3: Event creation (${app})`)
    ...
    builder.section('MOP 2.1.9: Joint forces to perform collaborative planning');
    const planningApps = ['JCC2 Cyber Ops', 'JCC2 Readiness'];
    planningApps.forEach(app => {
        builder.section(`MOP 2.1.9: Collaborative Planning (${app})`)
            .conditional(`exp_app_${toId(app)}`, 'not_equals', ['NA']);
    ...
    builder.section('MOP 2.3.2: JCC2 displays force disposition');
