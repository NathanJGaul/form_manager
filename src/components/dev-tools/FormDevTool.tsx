...
import {
  ...
  CSVIntegrityResult,
} from "../../utils/csvIntegrityChecker";
...
export const FormDevTool: React.FC<FormDevToolProps> = ({
  ...
  const [lastGeneratedData, setLastGeneratedData] = useState<Record<
    string,
    FormFieldValue
  > | null>(null);
  ...
  const handleExportAndValidate = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `form-dev-tool-export-${template.name}.csv`
      );
  ...
  const handleTestExportImport = async () => {
    ...
    try {
      ...
      // Decode it back
      const decodedData = (await decodeFromSharing(
        encodedString
      )) as FormInstance;
      ...
      if (isIdentical) {
        ...
        alert(
          "✅ Export/Import validation successful! The form data survived the encoding/decoding process intact."
        );
      } else {
        // Find differences
        ...
        for (const key in tempInstance.data) {
          if (
            JSON.stringify(tempInstance.data[key]) !==
            JSON.stringify(decodedData.data[key])
          ) {
            differences.push(
              `Field "${key}": Original=${JSON.stringify(
                tempInstance.data[key]
              )}, Decoded=${JSON.stringify(decodedData.data[key])}`
            );
          }
        }

        alert(
          `❌ Export/Import validation failed! Found ${
            differences.length
      ...
      console.log("Export/Import Test Results:", {
        ...
        compressionRatio: (
          encodedString.length / originalDataStr.length
        ).toFixed(2),
      });
    } catch (error) {
      console.error("Error during export/import test:", error);
      alert(
        `Export/Import test failed: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    } finally {
